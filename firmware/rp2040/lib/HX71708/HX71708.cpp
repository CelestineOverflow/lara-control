...
//     URL: https://github.com/RobTillaart/HX71708
...
void HX71708::begin(uint8_t dataPin, uint8_t clockPin)
{
  _dataPin  = dataPin;
  _clockPin = clockPin;
...
void HX71708::reset()
{
  ...
  _offset   = 0;
  _scale    = 1;
  _frequency     = HX71708_FREQUCUENCY_10;
  _lastRead = 0;
...
float HX71708::read()
{
  ...
  //  selection goes through the set_frequency(frequency)
  //
  uint8_t m = 1;
  if      (_frequency == HX71708_FREQUCUENCY_10) m = 1;
  else if (_frequency == HX71708_FREQUCUENCY_20)  m = 2;
  else if (_frequency == HX71708_FREQUCUENCY_80)  m = 3;
  else if (_frequency == HX71708_FREQUCUENCY_320) m = 4;
...
//  frequency
//
//  note: if parameter frequency == 0xFF40 some compilers
//  will map that to 0x40 == HX71708_CHANNEL_A_frequency_64;
//  solution: use uint32_t or larger parameters everywhere.
//  note that changing frequency/channel may take up to 400 ms (page 3)
bool HX71708::set_frequency(uint8_t frequency, bool forced)
{
  if ( (not forced) && (_frequency == frequency)) return true;
  switch(frequency)
  {
    case HX71708_FREQUCUENCY_10:
    case HX71708_FREQUCUENCY_20:
    case HX71708_FREQUCUENCY_80:
    case HX71708_FREQUCUENCY_320:
      _frequency = frequency;
      read();     //  next user read() is from right channel / frequency
      return true;
  }
...
void HX71708::set_offset(long offset)
{
  _offset = offset;
}


long HX71708::get_offset()
{
  return _offset;
}
