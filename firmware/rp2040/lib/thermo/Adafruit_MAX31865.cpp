...
float Adafruit_MAX31865::temperature(float RTDnominal, float refResistor) {
  float reading = readRTDNonBlocking();
  if (reading == 0xFFFF) {
    return NAN;
  }else{
    return calculateTemperature(reading, RTDnominal, refResistor);
  }
}
/**************************************************************************/
/*!
...
uint16_t Adafruit_MAX31865::readRTDNonBlocking(void) {
  ...
  switch (state) {
    case 0: // Clear faults and enable bias
      clearFault();
      enableBias(true);
    ...
    case 1: // Wait for bias stabilization
      if (currentMillis - lastCallTime >= 10) {
        uint8_t t = readRegister8(MAX31865_CONFIG_REG);
        t |= MAX31865_CONFIG_1SHOT;
        writeRegister8(MAX31865_CONFIG_REG, t);
        lastCallTime = currentMillis;
    ...
    case 2: // Wait for conversion to complete
      if (currentMillis - lastCallTime >= 65) {
  ...
  return 0xFFFF; // If still processing, return error code
}
